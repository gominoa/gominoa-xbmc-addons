...
//    CLog::Log(LOGDEBUG, "Video Visualization: Render");
    ...
    if (current >= m_nextFrameTime) { // We don't care to adjust for the exact timing
	m_nextFrameTime += m_secondsPerFrame;

	while (true) {
	    if (!m_decoder->nextFrame(m_texture)) { // End of video; restart
		m_decoder->seek(0.0);
		m_nextFrameTime = 0.0;
		continue;
            }
...
extern "C" bool ADDON_HasSettings() { return true; }
